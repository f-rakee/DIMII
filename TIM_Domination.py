...
def select_k_max(free_nodes, k, rr_sets):
    ...
    #    nx.write_gpickle(cover_graph, "graph/rr_sets_graph")
...
def calculate_theta(g, k):
    kpt_star, r_p = kpt_estimation(g, k)

    epsilon_p = 5.0 * np.power(1 * 0.01 / (k + 1), 1.0 / 3.0)

    kpt_plus = refine_kpt(g, k, kpt_star, epsilon_p, r_p)

    Lambda = get_lambda(g.number_of_nodes(), k)
